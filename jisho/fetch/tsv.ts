...
export function formatSenses(jishoData: Data) {
    return jishoData.senses
    .map(
        sense=>`${sense.english_definitions.join(" | ")}<small>(${sense.parts_of_speech})</small>`
    )
    .join("\<br/\>")
}
...
export async function getSlugCsv(slug: string) {
    try {
        ...
        const senses = formatSenses(main)
